...
impl Dataset {
    ...
    ///
    // An example has to name a vintage, and a vintage is a feature — so the example is
    // written only into a build that has the one it names. Without this, `cargo test`
    // under `--no-default-features --features cgmes2` failed on the documentation rather
    // than on the code.
    #[cfg_attr(
        feature = "cgmes3",
        doc = r#"
