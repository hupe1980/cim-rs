...
fn reads_the_microgrid_2_4_15_model() {
    let archives = match cas2_archives("MicroGrid/BaseCase_BC") {
        Some(a) => a,
        None => {
            eprintln!("skipping: CGMES 2.4.15 corpus not present; run scripts/fetch-specs.sh");
            return;
        }
    };
...
fn applying_a_difference_reclassifies_an_object() {
    ...
    for (i, archive) in diff_archives.iter().enumerate() {
        ...
        let pre_existing_conflicts = load.report.by_rule(cim::Rule::DuplicateMrid).count();

        for path in &diffs {
            let file = std::fs::File::open(path).unwrap();
            let Some(diff) =
                cim::reader::read_difference(SCHEMA, std::io::BufReader::new(file), None).unwrap()
            else {
                continue;
            };
        ...
        if pre_existing_conflicts > 0 {
            let after =
                cim::validate::validate_with(&ds, &cim::validate::ValidateOptions::essential());
            assert!(
                !after.has_errors(),
