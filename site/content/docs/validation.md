...
| Code | Rule | Meaning |
|---|---|---|
| `CIM0001` | `UnknownClass` | An element names a class the schema does not define |
| `CIM0002` | `UnknownAttribute` | A property the schema does not define, or one the object's class does not have |
| `CIM0003` | `InvalidValue` | A value could not be parsed as its declared type |
| `CIM0004` | `NonConformingMrid` | An identifier is not a UUID, or not written as 61970-552 requires |
| `CIM0005` | `DuplicateMrid` | One identifier used by two unrelated classes |
| `CIM0006` | `DanglingReference` | A reference points outside the dataset |
| `CIM0007` | `MissingRequired` | A mandatory attribute is absent |
| `CIM0008` | `CardinalityExceeded` | An attribute occurs more often than its multiplicity permits |
| `CIM0009` | `WrongReferenceTarget` | A reference points at an object of an incompatible class |
| `CIM0010` | `AttributeNotInProfile` | A file carries data belonging to a profile its header does not declare |
| `CIM0011` | `AbstractInstantiated` | An abstract class has an instance |
| `CIM0012` | `Deprecated` | A deprecated class or attribute is in use |
| `CIM0013` | `MalformedHeader` | The `md:FullModel` header is missing or incomplete |
| `CIM0014` | `UnsatisfiedDependency` | A header declares a `Model.DependentOn` that was not loaded |
| `CIM0015` | `Structure` | A structural problem that did not prevent reading |
| `CIM0016` | `DatatypeMismatch` | A stored value does not have the shape its attribute declares |
| `CIM0017` | `FixedValueMismatch` | An attribute the schema pins with `cims:isFixed` holds something else |
| `CIM0018` | `ConflictingValue` | Two loaded files gave one single-valued attribute two different values |
| `CIM0019` | `IllegalXmlCharacter` | A value holds a character XML 1.0 cannot represent in any form |
| `CIM0020` | `UnserializableIdentifier` | An identifier has no valid `rdf:ID` or `rdf:about` form |
...
production in neither direction, so a mis-encoded or corrupted source file used to travel
straight through and come out as something `xmllint`, Xerces and `lxml` all refuse. The
reader now strips the character and says so; the writer strips it too, because
`Object::set` is public; and this check names it in a model that already holds one.
...
Both were found the same way the duplicate `xmlns:md` was: by handing the output to a
parser that is not this crate's. The first had survived that check once already — the
well-formedness gate was built on `quick-xml` too, and inherited its tolerance. It now
tests the character range itself.
